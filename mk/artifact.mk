...
ifeq ($(call has, SYSTEM), 1)
  ...
  RV32EMU_PREBUILT_RELEASES_URL = https://github.com/sysprog21/rv32emu-prebuilt/releases/download/$(LATEST_RELEASE)
else
...
ifeq ($(call has, ARCH_TEST), 1)
	$(Q)if [ "$(RES)" = "1" ]; then \
	    ...
	    wget -q --show-progress $(RV32EMU_PREBUILT_RELEASES_URL)/$(RV32EMU_PREBUILT_TARBALL) -O build/$(RV32EMU_PREBUILT_TARBALL);\
	else \
	    $(call notice, [OK]); \
	fi
else
	$(Q)if [ "$(RES)" = "1" ]; then \
	    $(PRINTF) "\n$(YELLOW)SHA-1 verification failed! Re-fetching prebuilt binaries from \"rv32emu-prebuilt\" ...\n$(NO_COLOR)"; \
	    wget -q --show-progress $(RV32EMU_PREBUILT_RELEASES_URL)/$(RV32EMU_PREBUILT_TARBALL) -O- | tar -C build --strip-components=1 -xz; \
	else \
	    $(call notice, [OK]); \
	fi
endif
...
ifeq ($(call has, SYSTEM), 1)
	$(Q)wget -q -O $(BIN_DIR)/sha1sum-linux-image $(RV32EMU_PREBUILT_RELEASES_URL)/sha1sum-linux-image
	$(Q)$(call notice, [OK])
else ifeq ($(call has, ARCH_TEST), 1)
	$(Q)wget -q -O $(BIN_DIR)/rv32emu-prebuilt-sail-$(HOST_PLATFORM).sha $(RV32EMU_PREBUILT_RELEASES_URL)/rv32emu-prebuilt-sail-$(HOST_PLATFORM).sha
else
	$(Q)wget -q -O $(BIN_DIR)/sha1sum-linux-x86-softfp $(RV32EMU_PREBUILT_RELEASES_URL)/sha1sum-linux-x86-softfp
	$(Q)wget -q -O $(BIN_DIR)/sha1sum-riscv32 $(RV32EMU_PREBUILT_RELEASES_URL)/sha1sum-riscv32
	$(Q)$(call notice, [OK])
endif
endif
